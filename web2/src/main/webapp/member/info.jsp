...
<c:if test="${sessionScope.user == null }">
	<script> alert("로그인부터 하고와라 애송이"); history.back();</script>
</c:if>
<c:set var="img" value=""/>
<c:if test="${user.image != null }">
	<c:set var="img" value="style='background:url(http://192.168.0.26/static/image/${user.image }) no-repeat center; background-size:contain;'"/>
</c:if>
		...
		<div id="grid">
			...
			<c:forEach var="res" items="${ login_list }">	
					<li>
						...
						<span class="login_ip">${ res.getIp() }</span>
